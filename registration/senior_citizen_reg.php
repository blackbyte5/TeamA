<!-- Multistep Form -->
  ...
  <div id="first">
    ...
    <h2 class="title">Senior Citizen Requirements</h2>
    <div class="error">
      <?php
      ...
      $Comm_Serv =  '';
        ...
        $sqlsenior = "SELECT * FROM senior_application";
        $resultsenior = mysqli_query($conn, $sqlsenior);
        mysqli_num_rows($resultsenior);
        $rowssenior = mysqli_fetch_assoc($resultsenior);

        if (empty($lname) || empty($fname) || empty($Civil_Status) || empty($Gender)
            || empty($Cnumber) || empty($age) || empty($bplace) || empty($address) || empty($_FILES['pdf_file']['name'])) {
        ...
        elseif ($rowssenior['user_id'] == $userid) {
          echo "You can register only once<br>Wait for approval";
        }
          ...
          if(in_array($fileType, $allowTypes))
          {
            if(move_uploaded_file($_FILES['pdf_file']['tmp_name'], $targetFilePath))
            {
              ...
              $sql = "INSERT INTO senior_application (ControlNumber, user_id, Lastname, Firstname, MiddleName, CivilStatus, Gender, ContactNumber, Age, Birthday, BirthPlace, Address, DateFillUp)
                      VALUES ('$ctrl_num', '$userid','$lname', '$fname', '$mname', '$Civil_Status', '$Gender', '$Contactnumber', '$age', '$bday', '$bplace', '$address','$datefill')";

              $sql1 = "INSERT INTO senior_application_detail (ControlNumber, user_id, ComelicReg, HighestEducationalAttainment, Living_Residing, Occupation, SourceIncome, MonthlyIncome,
                                  Areas_Specialization, PROBorNEEDS_Economics, PROBorNEEDS_Social, PROBorNEEDS_Health, PROBorNEEDS_Housing, Comm_Serv)
                 VALUES ('$ctrl_num', '$userid', '$Com_Reg', '$hea', '$Living', '$Occupation', '$Source', '$MONTHLY', '$SKILLS', '$PROBorNEEDS_Econ',
                    '$PROBorNEEDS_Soc', '$PROBorNEEDS_H', '$PROBorNEEDS_House', '$Comm_Serv')";

              $sql2 = "INSERT INTO senior_upload_requirements (ControlNumber,user_id, Requirements, Lastname, Firstname, Middle_Name, Gender, Age)
...
<strong>Control Number:<a style="color:red;"> (Remember this control number)</a></strong><br><br>
  ...
  <input type="text" class="form-control" style="text-align:center; " name="ctrl_num" value="<?php echo date('md').(rand(0,10000)).date('y');?>" readonly><br><br>
  <a style="color:red">*</a><input type="text" class="form-control"  name="lname" placeholder="Last Name" pattern="^[a-zA-Z ]+$"  oninput="if (!this.checkValidity()) this.value = this.value.slice(0, -1)"value="<?php echo $lname;?>">
  <a style="color:red">*</a><input type="text" class="form-control"  name="fname" placeholder="First Name" pattern="^[a-zA-Z ]+$"  oninput="if (!this.checkValidity()) this.value = this.value.slice(0, -1)"value="<?php echo $fname;?>">
  ...
  <a style="color:red">*</a><br>
    <strong>LIVING/RESIDING WITH (check all applicable):</strong><br><br>
         ...
         if(isset($_POST['LIVING']) && in_array($LorR, $_POST['LIVING'])){
          ...
          echo '<label><input type="checkbox" value="'.$LorR.'" name="LIVING[]" '.$livcheck.'> '.$LorR.'</label><br>';
        }
        ?>
