<?php include 'navs/staff_navs.php'; ?>
  ...
  <input type="text" id="myInput2" onkeyup="myFunction2()" placeholder="Search for Name..">
    ...
    $sql = "SELECT * FROM approved_applications JOIN approved_upload_requirements
            ...
            JOIN approved_pwd_app_detail ON approved_pwd_app_detail.ControlNumber = approved_applications.ControlNumber
    ...
    while($row = mysqli_fetch_array($result)) {
      $id=$id+1;
       ...
       echo '<tr>
            ...
            <td>
            <a href = "applicants_pwd_approved.php?delete='.$row["ControlNumber"].'" class="btn btn-danger">DELETE</a>
            </td>
            <td>
              <a href = "applicants_pwd_approved_sendsms.php?sendsms='.$row["ControlNumber"].'" class="btn btn-warning">Send SMS</a>
            </td>
            <td>
              <a href = "applicants_pwd_approved_details.php?details='.$row["ControlNumber"].'" class="btn btn-info">DETAILS</a>
            </td>
            <td>
              <a href = "applicants_pwd_approved.php?undo='.$row["ControlNumber"].'" class="btn btn-primary">UNDO</a>
            </td>
            </tr>';
      }
    } else {
  ...
  if(isset($_GET['delete'])){
     ...
     $id = $_GET['delete'];
        ...
        $sql1 = "DELETE FROM approved_pwd_app_detail
                WHERE ControlNumber = ".$id;
      ...
      if(isset($_GET['undo'])){
              ...
              $sql1 = "INSERT INTO pwd_application_detail
                      SELECT * FROM approved_pwd_app_detail
                      WHERE ControlNumber=".$id;
              ...
              $sql2 = "INSERT INTO pwd_upload_requirements
                  ...
                  #---------------------------------------------Delete---------------------------------
                  $sq = "DELETE FROM approved_status
                          WHERE ControlNumber = ".$id;
