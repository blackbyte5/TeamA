...
<!-- Multistep Form -->
  ...
  <div id="first">
    ...
    <div class="error">
      ...
      $sql = "SELECT * FROM approved_pwd_app_detail WHERE ControlNumber=".$id;
      $result = mysqli_query($conn, $sql);
      ...
      $sql = "SELECT * FROM approved_upload_requirements WHERE ControlNumber=".$id;
      $result = mysqli_query($conn, $sql);
      $row = mysqli_fetch_assoc($result);
...
$PROBorNEEDS_Housing = '';
  ...
  $allowTypes = array('pdf');
    ...
    if(move_uploaded_file($_FILES['pdf_file']['tmp_name'], $targetFilePath))
    {
      ...
      $sql = "UPDATE approved_applications SET user_id='$id', Lastname='$lname', Firstname='$fname', MiddleName='$mname', CivilStatus='$Civil_Status', Gender='$Gender', ContactNumber='$Contactnumber', Age='$age', Birthday='$bday', BirthPlace='$bplace', Address='$address', DateFillUp='$datefill'
              WHERE  ControlNumber ='$ctrl_num'";

      $sql1 = "UPDATE approved_pwd_app_detail SET user_id='$id', PWD_Number='$Pwd_Num', ComelicReg='$Com_Reg', TypeOfDisability='$Type_disability', Inborn_Conginetal='$Inbo_conge', State_Illness='$If_ill', YearAccident='$yr_cause', HighestEducationalAttainment='$hea',Living_Residing='$Living', Occupation='$Occupation', SourceIncome='$Source', MonthlyIncome='$MONTHLY',
      ...
      $sql2 = "UPDATE approved_upload_requirements SET user_id='$id', Requirements='$Requirements', Lastname='$lname', Firstname='$fname', Middle_Name='$mname', Gender='$Gender', Age='$age'
              WHERE ControlNumber ='$ctrl_num'";
      ...
      $result2 = mysqli_query($conn, $sql2);
                  ...
                  if (!$result || !$result1 || !$result2) {
                    echo (mysqli_error($conn))."Data Cann't Update";
                  }else{
