<?php include 'navs/staff_navs.php'; ?>
  ...
  <form method="POST" action="<?php htmlspecialchars($_SERVER['PHP_SELF']); ?>">
    ...
    <?php
    $id = 0;
    $sql = "SELECT * FROM pwd_applications JOIN pwd_upload_requirements
            ON pwd_applications.ControlNumber = pwd_upload_requirements.ControlNumber
            JOIN pwd_application_detail ON pwd_application_detail.ControlNumber = pwd_applications.ControlNumber ORDER BY DateFillUp ASC";
    ...
    while($row = mysqli_fetch_array($result)) {
      $id=$id+1;
       ...
       echo '<tr>
            ...
            <td>'.$row["TypeOfDisability"].'</td>
            <td>
              <a href = "applicants_pwd_sendsms.php?sendsms='.$row["ControlNumber"].'" class="btn btn-warning">Send SMS</a>
            </td>
            <td>
              <a href = "applicants_pwd.php?approve='.$row["ControlNumber"].'" class="btn btn-success">APPROVE</a>
            </td>
            <td>
              <a href = "applicants_pwd.php?disapprove='.$row["ControlNumber"].'" class="btn btn-danger">DISAPPROVE</a>
            </td>
            <td>
              <a href = "applicants_pwd_details.php?details='.$row["ControlNumber"].'" class="btn btn-info">DETAILS</a>
            </td>
            </tr>';
      }
    } else {
...
if(isset($_GET['approve'])){
        ...
        $sql2 = "INSERT INTO approved_upload_requirements
            ...
            #---------------------------------------------Delete---------------------------------
            $del = "DELETE FROM pwd_applications
                    WHERE ControlNumber = ".$id;

            $del1 = "DELETE FROM pwd_application_detail
                    WHERE ControlNumber = ".$id;

            $del2 = "DELETE FROM pwd_upload_requirements
                    WHERE ControlNumber = ".$id;
...
//--------------------------------------------------------DISAPPROVE---------------------------
       if(isset($_GET['disapprove'])){
        ...
        $sql1 = "INSERT INTO disapproved_pwd_app_detail
                SELECT * FROM pwd_application_detail
                WHERE ControlNumber = ".$id;
      ...
      if($resul && $result && $result1 && $result2){
        ...
        #---------------------------------------------Delete---------------------------------
        $sql = "DELETE FROM pwd_applications
                WHERE ControlNumber = ".$id;

        $sql1 = "DELETE FROM pwd_application_detail
                WHERE ControlNumber = ".$id;

        $sql2 = "DELETE FROM pwd_upload_requirements
                WHERE ControlNumber = ".$id;
