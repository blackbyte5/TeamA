...
<!-- Multistep Form -->
  ...
  <div id="first">
    ...
    <h2 class="title">Submitted PWD Registration</h2>
    <div class="error">
      <?php
      if (isset($_GET['submitted'])) {
      $id = $_GET['submitted'];

      $sql = "SELECT * FROM pwd_applications WHERE user_id=".$id;
      ...
      $sql = "SELECT * FROM pwd_application_detail WHERE user_id=".$id;
      $result = mysqli_query($conn, $sql);
      $row = mysqli_fetch_array($result);

      $Com_Reg = $row['ComelicReg'];
      $Pwd_Num = $row['PWD_Number'];
      $Types_disability = $row['TypeOfDisability'];
      $Inbo_conge = $row['Inborn_Conginetal'];
      $If_ill = $row['State_Illness'];
      $yr_cause = $row['YearAccident'];
      ...
      $sql = "SELECT * FROM pwd_upload_requirements WHERE user_id=".$id;
      $result = mysqli_query($conn, $sql);
      $row = mysqli_fetch_assoc($result);
...
$PROBorNEEDS_Housing = '';
  ...
  if (isset($_POST['update'])) {
    ...
    if (empty($lname) || empty($fname) || empty($Civil_Status) || empty($Gender)
        || empty($Cnumber) || empty($age) || empty($bplace) || empty($address) || empty($Pwd_Num)
        || empty($Type_disability) || empty($age)|| empty($HEA) || empty($occupation) || empty($SOURCE) || empty($MONTHLY)|| empty($SKILLS) || empty($Comm_Serv)
  ...
  $allowTypes = array('pdf');
    ...
    if(move_uploaded_file($_FILES['pdf_file']['tmp_name'], $targetFilePath))
    {
      ...
      $sql = "UPDATE pwd_applications SET ControlNumber ='$ctrl_num', Lastname='$lname', Firstname='$fname', MiddleName='$mname', CivilStatus='$Civil_Status', Gender='$Gender', ContactNumber='$Contactnumber', Age='$age', Birthday='$bday', BirthPlace='$bplace', Address='$address', DateFillUp='$datefill'
              WHERE  user_id=".$id;

      $sql1 = "UPDATE pwd_application_detail SET ControlNumber ='$ctrl_num', PWD_Number='$Pwd_Num', ComelicReg='$Com_Reg', TypeOfDisability='$Type_disability', Inborn_Conginetal='$Inbo_conge', State_Illness='$If_ill', YearAccident='$yr_cause', HighestEducationalAttainment='$hea',Living_Residing='$Living', Occupation='$Occupation', SourceIncome='$Source', MonthlyIncome='$MONTHLY',
      ...
      $sql2 = "UPDATE pwd_upload_requirements SET ControlNumber ='$ctrl_num', Requirements='$Requirements', Lastname='$lname', Firstname='$fname', Middle_Name='$mname', Gender='$Gender', Age='$age'
              WHERE user_id=".$id;
...
<input type="hidden" class="form-control"  name="userid" value="<?php echo $id;?>" readonly>
  ...
  <input type="text" class="form-control"  name="Pwd_Num"  placeholder="PWD Number" pattern="^[0-9- ]+$" oninput="if (!this.checkValidity()) this.value = this.value.slice(0, -1)"value="<?php echo $Pwd_Num;?>"><br><br>
  ...
  <input type="text" class="form-control"  name="Cnumber" maxlength="10" placeholder="Contact Number"  pattern="^[pattern="^[a-zA-Z0-9,. ]+$" oninput="if (!this.checkValidity()) this.value = this.value.slice(0, -1)"0-9]+$" oninput="if (!this.checkValidity()) this.value = this.value.slice(0, -1)"value="<?php echo substr($cnumber, 3, 12);?>">
  <input type="text" class="formage"  name="age" placeholder="Age" maxlength="3" pattern="^[pattern="^[a-zA-Z0-9,. ]+$" oninput="if (!this.checkValidity()) this.value = this.value.slice(0, -1)"0-9]+$" oninput="if (!this.checkValidity()) this.value = this.value.slice(0, -1)"value="<?php echo $age;?>"><br><br>
  </div>
  <div class="form-inline"><br>
...
<div class="form-inline"><br>
  ...
  <div class="form-inline"><br>
    ...
    if ($Types_disability==$tod){
      $tods = 'selected';
    }else {
      $tods='';
    }
    echo '<option value="'.$tod.'" '.$tods.'>'.$tod.'</option>';
...
<div class="form-inline">
 ...
 <div class="form-inline">
       ...
       <input type="text" class="form-control"  name="HEA1"  placeholder="Others pls. specify" value="<?php if($HEAS==$EA){echo $HEAS;}else{echo '';}?>"><br>
</div><br>
    <strong>LIVING/RESIDING WITH (check all applicable):</strong><br><br>
       ...
       <div class="form-inline"><br>
       <input type="text" class="form-control"  name="LIVING1"  placeholder="Others pls. specify" pattern="^[a-zA-Z0 ]+$" oninput="if (!this.checkValidity()) this.value = this.value.slice(0, -1)"value="<?php echo $cont; ?>"><br>
     </div><br>
  <div class="form-inline"><br>
      <select class="form-control" name="occupation[]">
       ...
       <input type="text" class="form-control"  name="occupation1"  placeholder="Others pls. specify" pattern="^[a-zA-Z0 ]+$" oninput="if (!this.checkValidity()) this.value = this.value.slice(0, -1)"value="<?php if($occupation==$occ){echo $occupation;}else{echo '';};?>"><br>
       </div><br>
       <div class="form-inline"><br>
...
<div class="others">
   ...
   <strong>PROBLEM/NEEDS COMMONLY ENCOUNTERED (check all applicable):</strong><br><br>
      ...
      <div class="form-inline">
      <input type="text" class="form-control"  name="PROBorNEEDS_Economics1"  pattern="^[a-zA-Z ]+$" oninput="if (!this.checkValidity()) this.value = this.value.slice(0, -1)"placeholder="Others pls. specify" value="<?php echo $cont; ?>">
      </div><br>
