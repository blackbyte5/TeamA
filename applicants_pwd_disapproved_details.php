...
<!-- Multistep Form -->
  ...
  <div id="first">
    ...
    <h2 class="title">Submitted PWD Requirements</h2>
    <div class="error">
      <?php
      if (isset($_GET['details'])) {
      $id = $_GET['details'];

      $sql = "SELECT * FROM disapproved_applications WHERE ControlNumber=".$id;
      ...
      $sql = "SELECT * FROM disapproved_pwd_app_detail WHERE ControlNumber=".$id;
      $result = mysqli_query($conn, $sql);
      ...
      $sql = "SELECT * FROM disapproved_upload_requirements WHERE ControlNumber=".$id;
      $result = mysqli_query($conn, $sql);
      $row = mysqli_fetch_assoc($result);
...
$PROBorNEEDS_Housing = '';
  ...
  $allowTypes = array('pdf');
    ...
    if(move_uploaded_file($_FILES['pdf_file']['tmp_name'], $targetFilePath))
    {
      ...
      $sql = "UPDATE disapproved_applications SET user_id='$id', Lastname='$lname', Firstname='$fname', MiddleName='$mname', CivilStatus='$Civil_Status', Gender='$Gender', ContactNumber='$Contactnumber', Age='$age', Birthday='$bday', BirthPlace='$bplace', Address='$address', DateFillUp='$datefill'
              WHERE  ControlNumber ='$ctrl_num'";

      $sql1 = "UPDATE disapproved_pwd_app_detail SET user_id='$id', PWD_Number='$Pwd_Num', ComelicReg='$Com_Reg', TypeOfDisability='$Type_disability', Inborn_Conginetal='$Inbo_conge', State_Illness='$If_ill', YearAccident='$yr_cause', HighestEducationalAttainment='$hea',Living_Residing='$Living', Occupation='$Occupation', SourceIncome='$Source', MonthlyIncome='$MONTHLY',
      ...
      $sql2 = "UPDATE disapproved_upload_requirements SET user_id='$id', Requirements='$Requirements', Lastname='$lname', Firstname='$fname', Middle_Name='$mname', Gender='$Gender', Age='$age'
              WHERE ControlNumber ='$ctrl_num'";
...
<div class="form-inline">
 ...
 <input type="text" class="form-control"  size="30" name="If_ill" pattern="^[a-zA-Z ]+$" oninput="if (!this.checkValidity()) this.value = this.value.slice(0, -1)"  placeholder="If ILLNESS please State Cause" value="<?php echo $If_ill;?>">
</div><br>
 <div class="form-inline">
  <input type="text" class="form-control"  size="30" name="yr_cause" pattern="^[0-9]+$" oninput="if (!this.checkValidity()) this.value = this.value.slice(0, -1)"  placeholder="If ACCIDENT please State Year" value="<?php echo $yr_cause;?>">
 </div><br>
 <div class="form-inline">
  ...
  <div class="form-inline"><br>
      <select class="form-control" name="occupation[]">
       ...
       <input type="text" class="form-control"  name="occupation1"  placeholder="Others pls. specify" value="<?php if($occupation==$occ){echo $occupation;}else{echo '';};?>"><br>
       </div><br>
       <div class="form-inline"><br>
