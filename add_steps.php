   ...
   <form id="regForm" action="<?php htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST" enctype="multipart/form-data">
     <div class="modal-content">
       <div class="modal-header">
         <h4 class="modal-title"><center>Add Steps Files</center></h4>
       </div>
     <?php
     ...
     if (isset($_POST['add'])) {
       ...
       $targetDir = 'steps/';
       $steps=basename($_FILES['pdf_file']['name']);
       $targetFilePath = $targetDir . $steps;
       ...
       if(in_array($fileType, $allowTypes))
       {
         if(move_uploaded_file($_FILES['pdf_file']['tmp_name'], $targetFilePath))
         {
           ...
           $sql = "INSERT INTO add_steps (Steps)
                     VALUES ('$steps')";
      ...
      ?>
       ...
       <?php
       $sql = "SELECT * FROM add_steps ORDER BY id ASC";
       $result = mysqli_query($conn, $sql);
       ...
       $val = $row['Steps'];
         ...
         echo '<tr>
               ...
               <td>
                   <center><a href="add_steps.php?delete='.$id.'" class="btn btn-danger btn-sm">delete</a></center>
               </td>
               </tr>';
         }
        }else {
       ...
       $sql = "DELETE FROM add_steps WHERE id=".$id;
       $result = mysqli_query($conn, $sql);
       }
        ?>
      </tbody>
